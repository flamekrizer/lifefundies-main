          ...
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden">
                ...
                {card.label && (
                  <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 z-20 border-y border-white/25 bg-white/15 backdrop-blur-md px-2 py-1.5 shadow-[0_4px_20px_rgba(0,0,0,0.15)]">
                    <p className="text-white text-[0.7rem] md:text-xs font-semibold text-center leading-tight [text-shadow:0_1px_3px_rgba(0,0,0,0.6)]">
                      {card.label}
                    </p>
                  </div>
                )}
            ...
            if (onCardClick) {
              return (
                <button key={index} type="button" onClick={() => onCardClick(index)} className="fan-card block cursor-pointer text-left">
                  {image}
                </button>
              );
            }
